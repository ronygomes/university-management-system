...
public class KeyCloakJwtConverterTest {
    ...
    static WireMockExtension wireMock = WireMockExtension.newInstance()
            .options(WireMockConfiguration.options().port(KEYCLOAK_MOCK_SERVER_PORT))
            .build();
    ...
    void setup() throws IOException {
        ...
        wireMock.stubFor(WireMock.get(WireMock.urlEqualTo("/realms/ums/.well-known/openid-configuration"))
                .willReturn(WireMock.aResponse()
                        ...
                        .withBody(classpathBodyFile("openid-configuration.json"))));

        wireMock.stubFor(WireMock.get(WireMock.urlEqualTo("/realms/ums/protocol/openid-connect/certs"))
                .willReturn(WireMock.aResponse()
                        .withHeader(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON_VALUE)
                        .withStatus(HttpStatus.OK.value())
                        .withBody(classpathBodyFile("certs.json"))));

        converter = new KeyCloakJwtConverter();
    }

    private static String classpathBodyFile(String name) throws IOException {
        try (InputStream is = KeyCloakJwtConverterTest.class.getClassLoader().getResourceAsStream("__files/" + name)) {
            Assertions.assertNotNull(is, "Missing test resource __files/" + name);
